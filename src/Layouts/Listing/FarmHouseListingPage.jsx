...
const PlotListingPage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const { data } = await searchProperties({ sub_type: "farmhouse" });
        setProperties(data || []);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow py-10 px-4 sm:px-8 md:px-16">
        <h2 className="text-2xl font-bold text-center mb-6">Plot Listings</h2>
...
export default PlotListingPage;
